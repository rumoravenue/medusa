    ...
    return __generator(this, function (_e) {
        switch (_e.label) {
            case 0:
                ...
                if (!featureFlagRouter.isFeatureEnabled(isolate_product_domain_1.default.key)) return [3 /*break*/, 2];
                return [4 /*yield*/, listAndCountProductWithIsolatedProductModule(req, req.filterableFields, req.listConfig)];
            case 1:
...
function listAndCountProductWithIsolatedProductModule(req, filterableFields, listConfig) {
    return __awaiter(this, void 0, void 0, function () {
        var remoteQuery, productIdsFilter, variantIdsFilter, promises, salesChannelIdFilter, salesChannelService, priceListId, priceListService, discountConditionId, variables, query, _a, products, count;
            ...
            switch (_b.label) {
                case 0:
                    ...
                    if (salesChannelIdFilter) {
                        salesChannelService = req.scope.resolve("salesChannelService");
                        promises.push(salesChannelService
                            .listProductIdsBySalesChannelIds(salesChannelIdFilter)
                            .then(function (productIdsInSalesChannel) {
                            var filteredProductIds = productIdsInSalesChannel[salesChannelIdFilter];
                            if (filterableFields.id) {
                                filterableFields.id = Array.isArray(filterableFields.id)
                                    ? filterableFields.id
                                    : [filterableFields.id];
                                var salesChannelProductIdsSet_1 = new Set(filteredProductIds);
                                filteredProductIds = filterableFields.id.filter(function (productId) {
                                    return salesChannelProductIdsSet_1.has(productId);
                                });
                            }
                            filteredProductIds.map(function (id) { return productIdsFilter.add(id); });
                        }));
                    }
